...
import 'package:luck_chat/comps/styles.dart';
import 'package:luck_chat/comps/widgets.dart';
                ...
                builder: (context, snapshot) {
                  if(snapshot.hasData) {
                    if(snapshot.data!.docs.isEmpty){
                        ...
                        builder: (context,AsyncSnapshot<QuerySnapshot>snap) {
                          return !snap.hasData? Container(): ListView.builder(
                            ...
                            itemBuilder: (context, i) {
                              return ChatWidgets.messagesCard(snap.data!.docs[i]['sent_by'] !=widget.id,snap.data!.docs[i]['message'], DateFormat('hh:mm a').format(snap.data!.docs[i]['datetime'].toDate()));
                            },
                          );
                        }
                      );
                      ...
                      return Center(
                        child: Text('No conversion found', style: Styles.h1().copyWith(color: Colors.indigo.shade400),),
                      );
                    }
                  }else{
                    return const Center(
